        ...
        if len(current_entries_dict) > 0:
            for mac, entry in current_entries_dict.items():
                ...
                # This check allows to see, if an address is spoofed or multiple devices have the same IP
                    ...
                    if changes:
                        cursor.execute(
                            "UPDATE arp_entries SET mac = ?, ipv6 = ?, interface = ?, hostname = ?, timestamp = ? WHERE ipv4 = ?", (mac, ipv6, interface, hostname, time_now, ipv4)
                        )
                        logging.info(
                            "ARP - Changes detected! "
                            + (f"IPv4: {ipv4} | " if protocols == "all" or protocols == "ipv4_only" else "")
                ...
                # This check allows to see, if an address is spoofed or multiple devices have the same IP
                    ...
                    if changes:
                        cursor.execute(
                            "UPDATE arp_entries SET mac = ?, ipv4 = ?, interface = ?, hostname = ?, timestamp = ? WHERE ipv6 = ?", (mac, ipv4, interface, hostname, time_now, ipv6)
                        )
                        logging.info(
                            "ARP - Changes detected! "
                            + (f"IPv4: {ipv4} | " if protocols == "all" or protocols == "ipv4_only" else "")
