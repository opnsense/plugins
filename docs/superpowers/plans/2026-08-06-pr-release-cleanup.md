...
List releases with paginated `gh api`, flatten the response, require the exact regular expression `pr-<pull-number>-[0-9]+\.[0-9]+`, then call the exact-tag helper for each match. The exact-tag helper validates the complete development tag and calls `gh release delete <tag> --cleanup-tag --yes --repo <repository>`. Validate inputs before listing or deleting.
