...
ALLOW_UNSUPPORTED_SYSTEM=yes BATCH=yes NO_DEPENDS=yes OPTIONS_UNSET=DOCS "$make_command" -C "$ports_directory/dns/bind-tools" PORTSDIR="$ports_directory" package
...
ALLOW_UNSUPPORTED_SYSTEM=yes BATCH=yes NO_DEPENDS=yes OPTIONS_UNSET=DOCS "$make_command" -C "$ports_directory/dns/bind920" PORTSDIR="$ports_directory" package
