...
import dhcplease_watcher as watcher
from dhcpwatcher import lease
...
class TestBuildFqdn(unittest.TestCase):
    def test_simple(self):
        self.assertEqual(watcher.build_fqdn('laptop', 'home.arpa'), 'laptop.home.arpa.')

    def test_already_qualified(self):
        self.assertEqual(watcher.build_fqdn('laptop.home.arpa', 'home.arpa'), 'laptop.home.arpa.')

    def test_empty_suffix(self):
        self.assertEqual(watcher.build_fqdn('laptop', ''), 'laptop..')


class TestBuildForwardCommands(unittest.TestCase):
    def test_add_ipv4(self):
        addr = ipaddress.IPv4Address('1.2.3.4')
        cmds = watcher.build_forward_commands('add', addr, 'host.example.')
    ...
    def test_delete_ipv4(self):
        ...
        cmds = watcher.build_forward_commands('delete', addr, 'old.example.')
...
class TestBuildReverseCommands(unittest.TestCase):
    def test_add_ipv4(self):
        addr = ipaddress.IPv4Address('1.2.3.4')
        cmds = watcher.build_reverse_commands('add', addr, 'host.example.')
    ...
    def test_delete_ipv4(self):
        addr = ipaddress.IPv4Address('10.0.0.5')
        cmds = watcher.build_reverse_commands('delete', addr, 'old.example.')
...
class TestIsValidHostname(unittest.TestCase):
    def test_valid_simple(self):
        self.assertTrue(watcher._is_valid_hostname('laptop-01'))

    def test_valid_multilabel(self):
        self.assertTrue(watcher._is_valid_hostname('host.sub.domain'))

    def test_invalid_leading_dash(self):
        self.assertFalse(watcher._is_valid_hostname('-bad'))

    def test_invalid_trailing_dash(self):
        self.assertFalse(watcher._is_valid_hostname('bad-'))

    def test_invalid_empty(self):
        self.assertFalse(watcher._is_valid_hostname(''))
...
class TestNormalizeIscLease(unittest.TestCase):
    def test_valid(self):
        ...
        result = watcher.normalize_isc_lease(lease, '')
    ...
    def test_expired(self):
        ...
        self.assertIsNone(watcher.normalize_isc_lease(lease, ''))
    ...
    def test_falsey_client_hostname_is_rejected(self):
        ...
        for hostname in (None, False, 0):
            with self.subTest(hostname=hostname):
                invalid_lease = dict(lease, **{'client-hostname': hostname})
                self.assertIsNone(watcher.lease_helpers.normalize_isc_lease(invalid_lease))
                self.assertIsNone(watcher.normalize_isc_lease(invalid_lease, ''))
...
class TestNormalizeKeaLease(unittest.TestCase):
    def test_valid(self):
        ...
        result = watcher.normalize_kea_lease(lease, 'kea-dhcp4')
        self.assertIsNotNone(result)
    ...
    def test_ia_pd_filtered(self):
        ...
        self.assertIsNone(watcher.normalize_kea_lease(lease, 'kea-dhcp6'))
    ...
    def test_expired(self):
        ...
        self.assertIsNone(watcher.normalize_kea_lease(lease, 'kea-dhcp4'))
...
class TestLegacyAndRuntimeLeaseHelpers(unittest.TestCase):
    def setUp(self):
        self.active_isc_lease = {
            'ends': time.time() + 3600,
            'binding': 'active',
            'client-hostname': 'laptop.home.example.',
            'address': '192.0.2.10',
            'hardware': {'mac-address': 'AA:BB:CC:DD:EE:FF'},
        }

    def test_active_isc_lease_preserves_common_values_and_legacy_shape(self):
        legacy = watcher.normalize_isc_lease(self.active_isc_lease, 'home.example')
        runtime = watcher.lease_helpers.normalize_isc_lease(self.active_isc_lease)

        self.assertEqual(
            legacy,
            {
                'address': ipaddress.IPv4Address('192.0.2.10'),
                'hostname': 'laptop.home.example',
                'mac': 'aa:bb:cc:dd:ee:ff',
                'ends': self.active_isc_lease['ends'],
                'state': 'active',
                'source': 'isc-dhcp',
                'type': None,
            },
        ...
        self.assertEqual(
            {key: legacy[key] for key in runtime}, runtime,
        )

    def test_free_and_expired_isc_leases_are_rejected_by_both_helpers(self):
        free_lease = dict(self.active_isc_lease, binding='free')
        expired_lease = dict(self.active_isc_lease, ends=time.time() - 1)

        for lease in (free_lease, expired_lease):
            self.assertIsNone(watcher.normalize_isc_lease(lease, 'home.example'))
            self.assertIsNone(watcher.lease_helpers.normalize_isc_lease(lease))

    def test_kea_ia_pd_lease_is_rejected_by_both_helpers(self):
        lease = {
            'ip-address': '2001:db8::/64',
            'type': 'IA_PD',
            'hostname': 'router',
        ...
        self.assertIsNone(watcher.normalize_kea_lease(lease, 'kea-dhcp6'))
        self.assertIsNone(watcher.lease_helpers.normalize_kea_lease(lease, 'kea-dhcp6'))

    def test_ipv4_and_ipv6_command_helpers_have_their_current_outputs(self):
        for address, record_type in (
            (ipaddress.IPv4Address('192.0.2.10'), 'A'),
            (ipaddress.IPv6Address('2001:db8::10'), 'AAAA'),
        ):
            fqdn = 'host.home.example.'
            legacy_forward = watcher.build_forward_commands('add', address, fqdn)
            runtime_forward = watcher.lease_helpers.forward_commands('add', address, fqdn)
            legacy_reverse = watcher.build_reverse_commands('add', address, fqdn)
            runtime_reverse = watcher.lease_helpers.reverse_commands('add', address, fqdn)

            self.assertEqual(
                legacy_forward,
                ['update add host.home.example. 300 {} {}'.format(record_type, address)],
            )
            self.assertEqual(
                runtime_forward,
                ['update add host.home.example. 300 {} {}'.format(record_type, address)],
            )
            self.assertEqual(legacy_forward, runtime_forward)
            self.assertEqual(
                legacy_reverse,
                ['update add {}. 300 PTR host.home.example.'.format(address.reverse_pointer)],
            )
            self.assertEqual(runtime_reverse, legacy_reverse)

    def test_already_qualified_hostname_has_the_same_fqdn_in_both_helpers(self):
        hostname = 'host.home.example'
        suffix = 'home.example'

        self.assertEqual(watcher.build_fqdn(hostname, suffix), 'host.home.example.')
        self.assertEqual(
            watcher.lease_helpers.build_fqdn(hostname, suffix), 'host.home.example.'
        )

    def test_most_specific_reverse_zone_is_selected_by_both_paths(self):
        ...
        self.assertEqual(
            watcher.BindUpdater._reverse_zone({'reverse_zones': reverse_zones}, address),
            '30.20.10.in-addr.arpa.',
        )
        self.assertEqual(
            watcher.lease_helpers.select_reverse_zone(address, reverse_zones)['zone'],
            '30.20.10.in-addr.arpa.',
        )


class TestPackageHelperCompatibility(unittest.TestCase):
    def setUp(self):
    ...
    def test_entry_point_keeps_lease_helper_compatibility(self):
        address = ipaddress.IPv4Address('10.0.0.5')
        self.assertEqual(
            watcher.build_forward_commands('add', address, 'host.example.'),
            lease.forward_commands('add', address, 'host.example.'),
        )
        self.assertEqual(
            watcher.build_reverse_commands('add', address, 'host.example.'),
            lease.reverse_commands('add', address, 'host.example.'),
        )
        self.assertEqual(
            watcher.build_fqdn('host.example', 'example'),
            lease.build_fqdn('host.example', 'example'),
        )
        self.assertEqual(
            watcher._is_valid_hostname('host.example'),
            lease.is_valid_hostname('host.example'),
        )
        ...
        normalized = lease.normalize_isc_lease(isc_lease)
        self.assertIsNotNone(normalized)
        self.assertEqual(
            watcher.normalize_isc_lease(isc_lease, 'example'),
            dict(normalized, state='active', type=None),
        )
        ...
        normalized = lease.normalize_kea_lease(kea_lease, 'kea-dhcp4')
        self.assertEqual(
            watcher.normalize_kea_lease(kea_lease, 'kea-dhcp4'),
            dict(normalized, state='0', type=None),
        )

    def test_state_manager_package_contract(self):
        self.assertIs(watcher.StateManager, StateManager)
        state = {
...
class TestExtractedWatcherModules(unittest.TestCase):
    def test_config_entry_point_is_package_loader(self):
        self.assertIsNotNone(importlib.util.find_spec('dhcpwatcher.config'))
        watcher_config = importlib.import_module('dhcpwatcher.config')
        self.assertIs(watcher.load_config, watcher_config.load_config)
        self.assertIs(watcher.config_mtime, watcher_config.config_mtime)
    ...
    def test_config_loader_preserves_mapping_shape_and_defaults(self):
        ...
        with tempfile.NamedTemporaryFile(mode='w', delete=False) as config:
            config.write('[global]\n')
            config.write('[reverse-zones]\n')
            config.write('10.20.30.0/24 = 30.20.10.in-addr.arpa\n')
            config.write('[mapping]\n')
            config.write('dhcp_source = isc-dhcp\n')
            config.write('lease_subnet = 10.20.30.5/24\n')
            config.write('hostname_suffix = home.example\n')
            config.write('reverse_zone = 30.20.10.in-addr.arpa\n')
            config.write('tsigkey_name = test-key\n')
            config.write('tsigkey_algo = hmac-sha256\n')
            config.write('tsigkey_secret = c2VjcmV0\n')
            config_path = config.name
        try:
            watcher_config = importlib.import_module('dhcpwatcher.config')
            mappings = watcher_config.load_config(config_path)
        finally:
    ...
    def test_config_loader_accepts_inferred_ranges_and_subnets(self):
        with tempfile.NamedTemporaryFile(mode='w', delete=False) as config:
            config.write('[mapping]\n')
            config.write('dhcp_source = kea-dhcp4\n')
            config.write('lease_scopes = 10.20.30.75-10.20.30.225, 10.20.40.0/24\n')
            config.write('hostname_suffix = home.example\n')
            config.write('tsigkey_name = test-key\n')
            config.write('tsigkey_algo = hmac-sha256\n')
            config.write('tsigkey_secret = c2VjcmV0\n')
            config_path = config.name
        try:
            mappings = watcher.load_config(config_path)
        finally:
            os.unlink(config_path)
    ...
    def test_updater_entry_point_is_package_updater(self):
        self.assertIsNotNone(importlib.util.find_spec('dhcpwatcher.updater'))
        updater = importlib.import_module('dhcpwatcher.updater')
        self.assertIs(watcher.run_nsupdate, updater.run_nsupdate)
        self.assertIs(watcher.BindUpdater, updater.BindUpdater)

    def test_watcher_resolves_runner_patched_after_construction(self):
        mapping = {
        ...
        calls = []
        instance = watcher.Watcher(state_path='/tmp/dhcplease-state-test.json')
        original_run_nsupdate = watcher.run_nsupdate
        watcher.run_nsupdate = (
            lambda current_mapping, commands, zone=None:
            calls.append((commands, zone)) or True
        )
        try:
            self.assertTrue(instance.updater.add_records(mapping, lease))
        finally:
            watcher.run_nsupdate = original_run_nsupdate
...
class TestRunNsupdate(unittest.TestCase):
    ...
    def test_run_nsupdate_writes_server_zone_and_send(self, run):
        ...
        self.assertTrue(watcher.run_nsupdate(
            mapping,
...
class TestStateManager(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp()
        self.state_file = os.path.join(self.tmpdir, 'state.json')
        self.sm = watcher.StateManager(self.state_file)
...
class TestLoadConfig(unittest.TestCase):
    def test_ipv6_reverse_zone_is_parsed_with_its_full_network(self):
        with tempfile.NamedTemporaryFile(mode='w', delete=False) as config:
            config.write('[reverse-zones]\n')
            config.write('fd29:abc0:a250:ab10::/64 = 0.1.b.a.ip6.arpa\n')
            config.write('[mapping]\ndhcp_source = isc-dhcp\n')
            config.write('lease_scopes = 10.20.30.75-10.20.30.225\n')
            config.write('hostname_suffix = home.example\ntsigkey_name = key\n')
            config.write('tsigkey_algo = hmac-sha256\ntsigkey_secret = c2VjcmV0\n')
            config_path = config.name
        try:
...
class TestWatcherStartupReconcile(unittest.TestCase):
    ...
    def test_replays_existing_state_after_zone_regeneration(self):
        ...
        instance = watcher.Watcher(state_path=self.state_file)
        instance.mappings = {
            'mapping-1': {
                'dhcp_source': 'isc-dhcp',
...
class TestWatcherReconcile(unittest.TestCase):
    ...
    def test_failed_delete_remains_persisted_until_retry_succeeds(self):
        ...
        entry = watcher.StateManager.lease_to_state(
...
class TestBindUpdater(unittest.TestCase):
    def test_add_records_builds_correct_commands(self):
        ...
        calls = []
        original_run_nsupdate = watcher.run_nsupdate
        watcher.run_nsupdate = lambda current_mapping, commands, zone=None: calls.append((commands, zone)) or True
        try:
            self.assertTrue(
                watcher.BindUpdater(watcher.run_nsupdate).add_records(mapping, lease)
            )
        finally:
            watcher.run_nsupdate = original_run_nsupdate
    ...
    def test_delete_records_builds_correct_commands(self):
        ...
        try:
            self.assertTrue(
                watcher.BindUpdater(watcher.run_nsupdate).delete_records(mapping, lease)
            )
        finally:
            watcher.run_nsupdate = original_run_nsupdate
