...
def _run_nsupdate(mapping, commands, zone=None):
...
_runner_resolver = _run_nsupdate
_shutdown_checker = _shutdown_requested


def set_runner_resolver(resolver):
    """Set the runtime nsupdate resolver used by subsequently created watchers."""
    global _runner_resolver
    _runner_resolver = resolver


def set_shutdown_checker(checker):
    """Set the shutdown predicate used by subsequently created watchers."""
...
class Watcher:
    ...
    def __init__(self, config_path='/usr/local/etc/bind/dhcpwatcher.conf',
        ...
        self.updater = BindUpdater(run_nsupdate_func or _runner_resolver)
        self._shutdown_requested = shutdown_checker or _shutdown_checker
        self.sources = {}
        self.mappings = {}
        self.cached_leases = {}
